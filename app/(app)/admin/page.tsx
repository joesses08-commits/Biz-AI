    ...
    return (
      <div className="min-h-screen bg-bg-base flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-text-muted mt-2">You don't have permission to view this page.</p>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-6xl mx-auto">
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="text-text-muted text-sm mt-1">Manage customers and billing</p>
          </div>
          <button
            onClick={() => setShowForm(!showForm)}
            className="bg-white text-black font-bold px-6 py-2.5 rounded-lg hover:bg-bg-hover transition"
          >
            + New Customer
          </button>
        </div>
        ...
        {showForm && (
          <div className="bg-bg-elevated border border-bg-border rounded-xl p-6 mb-8">
            <h2 className="text-xl font-bold mb-4">Create New Customer</h2>
            <div className="grid grid-cols-2 gap-4 mb-4">
              <div>
                <label className="text-text-muted text-sm mb-1 block">Full Name *</label>
                <input
                  value={form.name}
                  onChange={(e) => setForm({ ...form, name: e.target.value })}
                  className="w-full bg-bg-elevated border border-bg-border rounded-lg px-4 py-2.5 text-white focus:outline-none focus:border-white"
                  placeholder="John Smith"
                />
              </div>
              <div>
                <label className="text-text-muted text-sm mb-1 block">Email *</label>
                <input
                  value={form.email}
              ...
              <div>
                <label className="text-text-muted text-sm mb-1 block">Company</label>
                <input
                  value={form.company}
              ...
              <div>
                <label className="text-text-muted text-sm mb-1 block">Plan</label>
                <select
                  value={form.plan}
            ...
            <div className="flex gap-3">
              <button
                ...
                disabled={saving}
                className="bg-white text-black font-bold px-6 py-2.5 rounded-lg hover:bg-bg-hover transition disabled:opacity-50"
              >
                {saving ? "Creating..." : "Create Customer"}
              </button>
              <button
                onClick={() => setShowForm(false)}
                className="bg-bg-elevated text-white px-6 py-2.5 rounded-lg hover:bg-bg-hover transition"
              >
                Cancel
              </button>
            </div>
        ...
        <div className="grid grid-cols-4 gap-4 mb-8">
          ...
          <div className="bg-bg-elevated border border-bg-border rounded-xl p-4">
            <p className="text-text-muted text-sm">Trial</p>
        ...
        <div className="bg-bg-elevated border border-bg-border rounded-xl overflow-hidden">
          ...
          {loading ? (
            <div className="p-6"><p className="text-text-muted">Loading...</p></div>
          ) : customers.length === 0 ? (
            <div className="p-6"><p className="text-text-muted">No customers yet. Create your first one above.</p></div>
          ) : (
            <table className="w-full">
              <thead className="bg-bg-elevated">
                <tr>
                  <th className="text-left px-6 py-3 text-text-muted text-sm font-medium">Customer</th>
                  <th className="text-left px-6 py-3 text-text-muted text-sm font-medium">Plan</th>
                  <th className="text-left px-6 py-3 text-text-muted text-sm font-medium">Status</th>
                  <th className="text-left px-6 py-3 text-text-muted text-sm font-medium">Joined</th>
                  <th className="text-left px-6 py-3 text-text-muted text-sm font-medium">Actions</th>
                </tr>
              </thead>
              <tbody>
                {customers.map((customer) => (
                  <tr key={customer.id} className="border-t border-bg-border hover:bg-bg-elevated/50">
                    ...
                    <td className="px-6 py-4">
                      ...
                      <p className="text-text-muted text-xs">{planPrice[customer.plan]}</p>
                    </td>
                    <td className="px-6 py-4">
                      <select
                        value={customer.status}
                        onChange={(e) => updateStatus(customer.id, e.target.value)}
                        className={`bg-transparent text-sm font-medium ${statusColor[customer.status] || "text-text-primary"} border border-bg-border rounded px-2 py-1`}
                      >
                    ...
                    <td className="px-6 py-4">
                      <button
                        ...
                        className="bg-bg-elevated hover:bg-bg-hover text-white text-xs font-medium px-3 py-1.5 rounded-lg transition"
                      >
                        Copy Payment Link
                      </button>
                    </td>
