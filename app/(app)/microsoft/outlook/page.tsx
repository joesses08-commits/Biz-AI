    ...
    return (
      <div className="min-h-screen bg-[#0a0a0a] flex items-center justify-center">
        <div className="text-center max-w-md">
          ...
          <p className="text-white/40 text-sm mb-8">Connect your Microsoft account to read Outlook emails.</p>
    ...
    <div className="h-screen bg-[#0a0a0a] text-white flex flex-col overflow-hidden">
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-white/[0.06] flex-shrink-0">
        <div className="flex items-center gap-3">
          ...
          {email && <span className="text-white/30 text-xs">{email}</span>}
          <span className="text-white/20 text-xs">{emails.length} messages</span>
        </div>
        <button onClick={load} className="text-white/40 hover:text-white/80 text-xs px-3 py-1.5 rounded-lg border border-white/10 hover:border-white/20 transition">
          Refresh
        </button>
      </div>
            ...
            <div className="flex-1 overflow-y-auto">
              {filtered.length === 0 ? (
                ...
                <div key={email.id} onClick={() => setSelected(email)}
                  ...
                  <p className={`text-[11px] truncate mb-0.5 ${email.isUnread ? "text-white/80 font-medium" : "text-white/40"}`}>{email.subject}</p>
              ...
              <div className="p-8 max-w-3xl">
                ...
                <div className="flex flex-col gap-1 mb-6 pb-6 border-b border-white/[0.06]">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <p className="text-sm font-medium text-white">{selected.fromName || selected.from}</p>
                      <p className="text-[11px] text-white/30">{selected.from}</p>
                    </div>
                  </div>
