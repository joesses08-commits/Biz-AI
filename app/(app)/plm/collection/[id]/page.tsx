        ...
        <div className="mt-1 space-y-px max-h-48 overflow-y-auto">
          {done.map((p: any) => (
            <button key={p.id} onClick={() => onNoteProduct(p)}
              className="w-full flex items-center gap-1.5 px-1.5 py-1 rounded hover:bg-bg-elevated transition text-left">
          ...
          {notDone.map((p: any) => (
            <button key={p.id} onClick={() => !p._noTrack && onNoteProduct(p)}
              ...
              {p.images?.[0] ? <img src={p.images[0]} alt="" className="w-4 h-4 rounded object-cover flex-shrink-0 opacity-40" />
                : <div className="w-4 h-4 rounded bg-bg-elevated flex-shrink-0" />}
          ...
          <div className="bg-bg-elevated border border-bg-border rounded-2xl w-full max-w-sm p-5 space-y-3" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="text-sm font-semibold text-text-primary">{noteProduct.name}</p>
                  {noteProduct.sku && <p className="text-[10px] text-text-muted font-mono">{noteProduct.sku}</p>}
                </div>
              </div>
              <button onClick={() => setNoteProduct(null)} className="text-text-muted hover:text-text-secondary"><X size={14} /></button>
            </div>
            {noteProduct.notes && <div><p className="text-[10px] text-text-muted uppercase tracking-widest mb-1">Notes</p><p className="text-xs text-text-secondary leading-relaxed">{noteProduct.notes}</p></div>}
            {noteProduct.factory_notes && <div><p className="text-[10px] text-text-muted uppercase tracking-widest mb-1">Factory Notes</p><p className="text-xs text-text-secondary leading-relaxed">{noteProduct.factory_notes}</p></div>}
            {!noteProduct.notes && !noteProduct.factory_notes && <p className="text-xs text-text-muted italic">No notes</p>}
            <button onClick={() => { setNoteProduct(null); router.push(`/plm/${noteProduct.id}`); }} className="w-full py-2 rounded-xl bg-white/[0.06] border border-bg-border text-text-secondary text-xs hover:text-white/80 transition">Open Product →</button>
          </div>
        </div>
      )}
          ...
          <div className="bg-bg-elevated border border-bg-border rounded-2xl p-6 w-full max-w-lg space-y-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-text-secondary mt-0.5">Select products and send email notifications</p>
              </div>
              <button onClick={() => setBulkDisqualifyModal(null)} className="text-text-muted hover:text-text-secondary text-lg">✕</button>
            </div>
            <div>
              ...
              <div className="grid grid-cols-3 gap-2">
                {[{ key: "price", label: "Price", icon: "💰" }, { key: "speed", label: "Speed", icon: "⏱" }, { key: "quality", label: "Quality", icon: "⚠️" }].map(r => (
                  <button key={r.key} onClick={() => setBulkDisqualifyReason(r.key)}
                    className={`py-2.5 rounded-xl border text-xs font-semibold transition ${bulkDisqualifyReason === r.key ? "border-red-500/40 bg-red-500/10 text-red-400" : "border-bg-border text-text-secondary hover:border-white/20"}`}>
                    {r.icon} {r.label}
                  </button>
                ))}
              </div>
            ...
            <div>
              <p className="text-[10px] text-text-muted uppercase tracking-widest mb-1.5">Select Products</p>
              <div className="space-y-1 max-h-48 overflow-y-auto">
                {products.filter((p: any) => (p.plm_factory_tracks || []).some((t: any) => t.factory_id === bulkDisqualifyModal.factory.id && t.status === "active")).map((p: any) => {
                  ...
                  return (
                    <label key={p.id} className={`flex items-center gap-2 px-3 py-2 rounded-lg ${hasSampleRequested ? "hover:bg-bg-elevated cursor-pointer" : "opacity-40 cursor-not-allowed"}`}>
                      ...
                      {!hasSampleRequested && <span className="text-[9px] text-text-muted ml-auto">No sample requested</span>}
                    </label>
                  );
                })}
              </div>
            ...
            <div>
              <p className="text-[10px] text-text-muted uppercase tracking-widest mb-1.5">Internal Note (optional)</p>
            ...
            <div className="flex gap-2">
              ...
              <button onClick={() => setBulkDisqualifyModal(null)} className="px-4 py-2.5 rounded-xl border border-bg-border text-text-secondary text-sm">Cancel</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showRfqModal && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4 overflow-y-auto">
          <div className="bg-bg-elevated border border-bg-border rounded-2xl w-full max-w-lg p-6 space-y-4 my-4">
            ...
            <p className="text-xs text-text-secondary">{rfqProductIds.length} products from this collection</p>
            <div>
              <p className="text-[10px] text-text-muted uppercase tracking-widest mb-2">Include in sheet</p>
            ...
            <div>
              <p className="text-[10px] text-text-muted uppercase tracking-widest mb-2">Ask factories to fill in</p>
              <div className="grid grid-cols-2 gap-2">
            ...
            <div className="flex gap-2">
              ...
              <button onClick={() => setShowRfqModal(false)} className="px-4 rounded-xl border border-bg-border text-text-muted text-xs">Cancel</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showSampleModal && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4 overflow-y-auto">
          <div className="bg-bg-elevated border border-bg-border rounded-2xl w-full max-w-xl p-6 space-y-4 my-4">
            ...
            <p className="text-xs text-text-secondary">{sampleProductIds.length} products · all factories pre-selected.</p>
            <div className="space-y-2 max-h-80 overflow-y-auto">
              {sampleProductIds.map(pid => {
                ...
                return (
                  <div key={pid} className="border border-bg-border rounded-xl p-3 space-y-2">
            ...
            <div>
              ...
              <textarea value={sampleNote} onChange={e => setSampleNote(e.target.value)} rows={2} placeholder="e.g. Priority samples needed by May 1st"
                className="w-full bg-bg-elevated border border-bg-border rounded-xl px-3 py-2 text-white/70 placeholder-white/20 text-xs focus:outline-none resize-none" />
            </div>
            <div className="flex gap-2">
              <button onClick={async () => {
          ...
          <div className="flex items-start justify-between flex-wrap gap-4">
            <div>
              <div className="flex items-center gap-3 mb-1 flex-wrap">
                <div className="w-8 h-8 rounded-xl bg-bg-elevated border border-bg-border flex items-center justify-center">
                  <Layers size={14} className="text-text-secondary" />
                </div>
                <h1 className="text-2xl font-bold">{collection.name}</h1>
                {collection.season && <span className="text-xs text-text-muted bg-bg-elevated px-2 py-0.5 rounded-full">{collection.season} {collection.year}</span>}
            ...
            {products.map((p: any) => {
              ...
              return (
                <div key={p.id} className="flex items-center gap-4 border border-bg-border rounded-xl px-4 py-3 bg-bg-surface hover:border-bg-border transition">
                  {p.images?.[0] ? <img src={p.images[0]} alt="" className="w-10 h-10 rounded-lg object-cover flex-shrink-0 border border-bg-border" /> : <div className="w-10 h-10 rounded-lg bg-bg-elevated border border-bg-border flex-shrink-0" />}
