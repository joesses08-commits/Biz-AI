...
import {
  Package, Plus, Loader2, Check, X, ChevronDown, ChevronUp,
  LogOut, Layers, FileText, Send, ShoppingCart
} from "lucide-react";
...
const STAGE_LABELS: Record<string, string> = {
  ...
  revision_requested: "Revision Requested", sample_production: "Sample Production",
  sample_complete: "Sample Complete", sample_shipped: "Sample Shipped", sample_arrived: "Sample Arrived",
  killed: "Killed", design_brief: "Design Brief",
};
    ...
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
      <div className="bg-[#111] border border-white/10 rounded-2xl w-full max-w-sm p-6 space-y-4">
        <div>
          ...
          <p className="text-xs text-white/30 mt-0.5">Required for approvals, status changes, and kills.</p>
        </div>
        <div>
...
function PinPrompt({ onConfirm, onCancel }: { onConfirm: (pin: string) => void; onCancel: () => void }) {
  const [pin, setPin] = useState("");
  return (
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
      <div className="bg-[#111] border border-white/10 rounded-2xl w-full max-w-sm p-6 space-y-4">
...
function SamplesTab({ products, factories, token, onRefresh }: { products: any[]; factories: any[]; token: string; onRefresh: () => void }) {
  ...
  const eligible = products.filter(p => !p.killed &&
    ["ready_for_quote", "quotes_received", "artwork_sent", "concept"].includes(p.current_stage || "") &&
    !(p.plm_sample_requests || []).some((r: any) => r.status === "requested")
  );
  ...
  if (eligible.length === 0) return (
    <div className="text-center py-20 border border-white/[0.06] rounded-2xl">
      ...
      <p className="text-white/10 text-xs mt-1">Products with no active sample requests will appear here</p>
    </div>
  );

  return (
    <div className="space-y-3">
      <p className="text-xs text-white/30">Select factories and request samples for products.</p>
        ...
        return (
          <div key={product.id} className="border border-white/[0.07] rounded-2xl p-4 space-y-3">
            <div className="flex items-center gap-3">
              {product.images?.[0] && <img src={product.images[0]} alt="" className="w-9 h-9 rounded-xl object-cover flex-shrink-0" />}
            ...
            <input value={notes[product.id] || ""} onChange={e => setNotes(prev => ({ ...prev, [product.id]: e.target.value }))}
              placeholder="Note to factory (optional)"
            ...
            <button onClick={() => requestSample(product.id)} disabled={!selected.length || requesting === product.id}
              className="flex items-center gap-2 px-4 py-2 rounded-xl bg-amber-500 text-black text-xs font-semibold disabled:opacity-40 transition">
              {requesting === product.id ? <Loader2 size={10} className="animate-spin" /> : <Plus size={10} />}
              Request Samples from {selected.length} {selected.length === 1 ? "Factory" : "Factories"}
            </button>
          </div>
        );
      })}
...
export default function DesignerView({ portalUser, router }: { portalUser: any; router: any }) {
  ...
  const [activeTab, setActiveTab] = useState<"products" | "samples" | "prioritization" | "workflows">("products");
  ...
  const doWithPin = async (action: (pin: string) => Promise<void>) => {
    const pin = await getPin();
    if (!pin) return;
    setPinError("");
    try { await action(pin); }
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white">
      ...
      {pinPrompt && (
        <PinPrompt
          onConfirm={pin => { pinPrompt.resolve(pin); setPinPrompt(null); }}
          onCancel={() => { pinPrompt.resolve(""); setPinPrompt(null); }}
        />
      )}
        ...
        <div className="flex items-center gap-2">
          {!hasPinSet && (
            <button onClick={() => setShowSetPin(true)}
              className="text-xs text-amber-400 border border-amber-500/30 bg-amber-500/10 px-3 py-1.5 rounded-lg hover:bg-amber-500/20 transition">
              Set PIN
            </button>
          )}
      ...
      <div className="border-b border-white/[0.06] px-6">
        <div className="flex gap-0">
          {([["products", "Products"], ["samples", "Samples"], ["prioritization", "Prioritization"], ["workflows", "Workflows"]] as const).map(([key, label]) => (
      ...
      <div className="max-w-4xl mx-auto px-6 py-6">
        {loading ? (
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between gap-3 flex-wrap">
              <input value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
                placeholder="Search products..." className="bg-white/[0.03] border border-white/[0.08] rounded-xl px-3 py-2 text-white/70 placeholder-white/20 text-xs focus:outline-none w-56" />
              <div className="flex items-center gap-2">
            ...
            {showNewCollection && (
              <div className="border border-white/[0.08] rounded-2xl p-4 space-y-3 bg-white/[0.01]">
                ...
                <div className="grid grid-cols-3 gap-3">
                  <div><label className={lc}>Name *</label><input value={newCollection.name} onChange={e => setNewCollection({ ...newCollection, name: e.target.value })} placeholder="Summer 2026" className={ic} /></div>
                  <div><label className={lc}>Season</label><input value={newCollection.season} onChange={e => setNewCollection({ ...newCollection, season: e.target.value })} placeholder="Summer" className={ic} /></div>
            ...
            {filtered.length === 0 ? (
              ...
              <div className="space-y-2">
                {filtered.map(product => {
                  ...
                  return (
                    <div key={product.id} className={`border rounded-2xl overflow-hidden transition ${isKilled ? "border-red-500/20 opacity-60" : isHold ? "border-amber-500/20" : "border-white/[0.07] bg-white/[0.01]"}`}>
                      {/* Card header */}
                          ...
                          {!isKilled && (
                            <div>
                              ...
                              <div className="flex flex-wrap gap-1.5">
                                {DEV_STAGES.map(s => (
                                  <button key={s.key}
                                    onClick={() => fetch("/api/portal/designer", { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${tok()}` },
                                    ...
                                    <div key={sr.id} className="border border-white/[0.06] rounded-xl p-3 space-y-2">
                                      ...
                                      <div className="flex gap-1.5 flex-wrap">
                                        <button onClick={() => updateSampleOutcome(sr.id, product.id, sr.factory_id, currentStage, "approved")}
                                          className="text-[10px] px-2.5 py-1 rounded-lg bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 hover:bg-emerald-500/30 transition">
                                          ✓ Approve
                                        </button>
                                        <button onClick={async () => {
                                          ...
                                          className="text-[10px] px-2.5 py-1 rounded-lg bg-amber-500/20 text-amber-400 border border-amber-500/30 hover:bg-amber-500/30 transition">
                                          Revision
                                        </button>
                                        <button onClick={async () => {
                                          ...
                                          className="text-[10px] px-2.5 py-1 rounded-lg bg-red-500/20 text-red-400 border border-red-500/30 hover:bg-red-500/30 transition">
                                          Kill
                                        </button>
                                      </div>
                                    </div>
                            ...
                            <div>
                              ...
                              <div className="space-y-1.5">
                                {batches.map((b: any) => {
                                  ...
                                  return (
                                    <div key={b.id} className="flex items-center justify-between px-3 py-2 rounded-xl border border-white/[0.06] bg-white/[0.01]">
                                      <div>
                                        <p className="text-xs font-medium text-white/60">Order #{b.batch_number}</p>
                  ...
                  {factories.map(f => {
                    ...
                    return (
                      <button key={f.id} onClick={() => setPrioActiveFactory(f.id)}
                        ...
                        {f.name}
                        <span className={`text-[10px] px-1.5 py-0.5 rounded-full ${prioActiveFactory === f.id ? "bg-black/10" : "bg-white/[0.06]"}`}>{count} pending</span>
                      </button>
                    );
                  })}
                </div>
                          ...
                          {orderedSamples.map((sample: any, idx: number) => {
                            ...
                            return (
                              <div key={sample.id} draggable
                                ...
                                className={`flex items-center gap-4 px-4 py-3 rounded-xl border transition cursor-grab active:cursor-grabbing ${dragOver === sample.id ? "border-blue-500/40 bg-blue-500/5" : "border-white/[0.08] bg-white/[0.02]"}`}>
                                <div className="w-7 h-7 rounded-lg bg-blue-500/20 border border-blue-500/30 flex items-center justify-center text-xs font-bold text-blue-400 flex-shrink-0">{idx + 1}</div>
                                ...
                                <div className="flex flex-col gap-0.5 flex-shrink-0 opacity-30">
                                  {[0,1,2].map(i => <div key={i} className="w-4 h-0.5 bg-white/40 rounded" />)}
                                </div>
                              </div>
                            );
                          })}
          ...
          /* Workflows tab */
          <div className="space-y-4">
            <p className="text-xs text-white/30">Access key workflows from here.</p>
            <div className="grid grid-cols-1 gap-3">
              {[
                { icon: Send, name: "Factory Quote Request", desc: "Upload product list → get quotes from factories", color: "#3b82f6", href: "/workflows/factory-quote" },
                { icon: FileText, name: "PO Generator", desc: "Create and send purchase orders to factories", color: "#10b981", href: "/workflows/po-generator" },
              ].map(w => (
                <a key={w.name} href={w.href} target="_blank" rel="noopener noreferrer"
                  className="flex items-center gap-4 p-4 rounded-2xl border border-white/[0.07] bg-white/[0.01] hover:border-white/20 hover:bg-white/[0.03] transition">
                  <div className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0" style={{ background: `${w.color}15`, border: `1px solid ${w.color}30` }}>
                    <w.icon size={16} style={{ color: w.color }} />
                  </div>
                  <div>
                    <p className="text-sm font-semibold text-white">{w.name}</p>
                    <p className="text-xs text-white/30 mt-0.5">{w.desc}</p>
                  </div>
                  <div className="ml-auto text-white/20 text-xs">Opens in Jimmy →</div>
                </a>
              ))}
            </div>
            <div className="border border-white/[0.06] rounded-2xl p-4 bg-white/[0.01]">
              <p className="text-xs text-white/30 mb-3">Sample Requests</p>
              <p className="text-xs text-white/50">Go to the <button onClick={() => setActiveTab("samples")} className="text-blue-400 underline">Samples tab</button> to request samples from factories for specific products.</p>
            </div>
          </div>
        )}
      </div>
