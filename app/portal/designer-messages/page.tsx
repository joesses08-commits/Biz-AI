    ...
    <div className="flex h-screen bg-bg-base text-white overflow-hidden">
      <div className="w-72 flex-shrink-0 border-r border-bg-border flex flex-col">
        <div className="px-5 py-4 border-b border-bg-border flex items-center gap-3">
          <button onClick={() => router.push("/portal/dashboard")} className="text-text-muted hover:text-text-secondary"><ArrowLeft size={14} /></button>
          <p className="text-sm font-semibold">Messages</p>
        </div>
        <div className="flex-1 overflow-y-auto">
          {loading ? <p className="text-xs text-text-muted text-center py-8">Loading...</p> :
           sortedChats.length === 0 ? <p className="text-xs text-text-muted text-center py-8">No messages yet</p> :
           sortedChats.map(chat => (
            <div key={chat.track_id} onClick={() => openChat(chat)}
              ...
              {chat.product_image ? <img src={chat.product_image} className="w-10 h-10 rounded-xl object-cover border border-bg-border flex-shrink-0" /> :
                <div className="w-10 h-10 rounded-xl bg-bg-elevated border border-bg-border flex items-center justify-center flex-shrink-0"><span className="text-text-muted text-xs">📦</span></div>}
              ...
              <button onClick={e => togglePin(chat, e)} className="text-text-muted hover:text-text-secondary flex-shrink-0">
        ...
        <div className="flex-1 flex flex-col min-w-0">
          <div className="px-6 py-4 border-b border-bg-border flex items-center gap-3 flex-shrink-0">
            {activeChat.product_image ? <img src={activeChat.product_image} className="w-9 h-9 rounded-xl object-cover border border-bg-border" /> :
              <div className="w-9 h-9 rounded-xl bg-bg-elevated border border-bg-border flex items-center justify-center"><span className="text-xs">📦</span></div>}
            <div>
            ...
            {messages.map((msg: any, idx: number) => {
              ...
              return (
                ...
                return colors[Math.abs(hash) % colors.length];
                      ...
                      {msg.attachment_url && msg.attachment_type === "image" && <img src={msg.attachment_url} className="mt-2 max-w-xs rounded-xl border border-bg-border cursor-pointer" onClick={() => window.open(msg.attachment_url, "_blank")} />}
          ...
          <div className="px-6 py-4 border-t border-bg-border flex gap-2 items-end flex-shrink-0">
            <button onClick={() => fileInputRef.current?.click()} className="text-text-muted hover:text-text-secondary pb-2"><Paperclip size={16} /></button>
