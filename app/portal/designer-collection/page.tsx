          ...
          {done.map((p: any) => (
            <button key={p.id} onClick={() => onNoteProduct(p)}
              ...
              {p.images?.[0] ? <img src={p.images[0]} alt="" className="w-4 h-4 rounded object-cover flex-shrink-0" />
                : <div className="w-4 h-4 rounded bg-white/[0.06] flex-shrink-0" style={{ background: p.color || undefined }} />}
          ...
          {notDone.map((p: any) => (
            <button key={p.id} onClick={() => !p._noTrack && onNoteProduct(p)}
              ...
              {p.images?.[0] ? <img src={p.images[0]} alt="" className="w-4 h-4 rounded object-cover flex-shrink-0 opacity-40" />
                : <div className="w-4 h-4 rounded bg-white/[0.04] flex-shrink-0" style={{ background: p.color || undefined, opacity: 0.4 }} />}
...
export default function DesignerCollectionPage() {
  ...
  const load = async () => {
    ...
    if (data.products) {
      ...
      setAllFactories(getAllFactories(filteredProducts));
    ...
    if (data.factories) {
      // Use all factories for sample requests
      setAllFactories(data.factories);
    }
    setLoading(false);
  };
    ...
    <div className="min-h-screen bg-[#0a0a0a] text-white">
              ...
              <div className="grid grid-cols-2 gap-2">
                {[["name","Product Name"],["sku","SKU"],["description","Description"],["specs","Specifications"],["images","Image URLs"],["category","Category"],["notes","Notes"]].map(([key, label]) => (
                  <label key={key} className="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" checked={rfqInclude.includes(key)} onChange={e => setRfqInclude(prev => e.target.checked ? [...prev, key] : prev.filter(k => k !== key))} />
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {[["price","Unit Price"],["moq","MOQ"],["lead_time","Lead Time"],["sample_lead_time","Sample Lead Time"],["payment_terms","Payment Terms"],["sample_price","Sample Price"],["packaging","Packaging"],["notes","Notes/Comments"]].map(([key, label]) => (
                  <label key={key} className="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" checked={rfqAskFor.includes(key)} onChange={e => setRfqAskFor(prev => e.target.checked ? [...prev, key] : prev.filter(k => k !== key))} />
                    <span className="text-xs text-white/60">{label}</span>
                  </label>
                ))}
              </div>
            </div>
            {rfqDone && <div className="flex items-center gap-2 bg-emerald-500/10 border border-emerald-500/20 rounded-xl px-4 py-3"><span className="text-xs text-emerald-300">✓ RFQ created!</span></div>}
            <div className="flex gap-2">
              <button onClick={async () => {
                setCreatingRfq(true);
                const res = await fetch("/api/plm/rfq", { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${getToken()}` }, body: JSON.stringify({ product_ids: rfqProductIds, include: rfqInclude, ask_for: rfqAskFor }) });
                const data = await res.json();
                if (data.file_base64) {
                    ...
                    {isExpanded && (
                      <div className="flex flex-wrap gap-1.5 pl-8">
                        {allFactories.map((f: any) => (
                          <button key={f.id} onClick={() => setSampleSelections(prev => ({ ...prev, [pid]: sel.includes(f.id) ? sel.filter(fid => fid !== f.id) : [...sel, f.id] }))}
            ...
            <div className="flex gap-2">
              <button onClick={async () => {
                ...
                for (const pid of sampleProductIds) {
                  ...
                  await fetch("/api/portal/designer", { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${getToken()}` }, body: JSON.stringify({ action: "create_sample_requests", product_id: pid, factory_ids: factoryIds, note: sampleNote }) });
                }
                setRequestingSamples(false); setShowSampleModal(false); setSampleNote(""); load();
      ...
      <div className="border-b border-white/[0.06] px-8 py-6">
        <div className="max-w-full mx-auto">
          <button onClick={() => router.push("/portal")} className="flex items-center gap-1.5 text-xs text-white/30 hover:text-white/60 transition mb-4">
          ...
          <div className="flex items-start justify-between flex-wrap gap-4">
            ...
            <div className="flex items-center gap-2">
              <button onClick={openRfq} className="flex items-center gap-2 text-xs px-4 py-2 rounded-xl border border-pink-500/30 text-pink-400 font-semibold hover:bg-pink-500/10 transition">
      ...
      <div className="px-8 py-8 space-y-8 max-w-full overflow-x-auto">
        ...
        {/* Products List */}
        <div>
          <p className="text-[10px] text-white/25 uppercase tracking-widest mb-4">Products in Collection ({products.length})</p>
            ...
            {products.map((p: any) => {
              ...
              return (
                <div key={p.id} className="flex items-center gap-4 border border-white/[0.06] rounded-xl px-4 py-3 bg-white/[0.01] hover:border-white/10 transition">
                  {p.images?.[0] ? <img src={p.images[0]} alt="" className="w-10 h-10 rounded-lg object-cover flex-shrink-0 border border-white/[0.06]" /> 
                    : <div className="w-10 h-10 rounded-lg border border-white/[0.06] flex-shrink-0" style={{ background: p.color || "#1a1a1a" }} />}
